...
###Flexible function to read in data from the chess.com API with various specifications and at various levels###
queryChess <- function(category,
                       chessType,
                       username,
                       year,
                       month,
  ...
  if (category == "leaderboards") {
    ...
    URL <- paste("https://api.chess.com/pub/",
                 category,
                 "/",
                 username,
                 "/",
    ...
    parsed <- fromJSON(rawToChar(data$content))
    if (chessType == "All") {
      chessInfo1 <- as_tibble_row(unlist(parsed$chess_rapid)) |> mutate(type =
                                                                          "Rapid")
      chessInfo2 <- as_tibble_row(unlist(parsed$chess_blitz)) |> mutate(type =
                                                                          "Blitz")
      chessInfo3 <- as_tibble_row(unlist(parsed$chess_bullet)) |> mutate(type =
                                                                           "Bullet")
      chessInfo <- bind_rows(chessInfo1, chessInfo2, chessInfo3) |>
        mutate(
          last.rating = as.integer(last.rating),
          last.date = as.integer(last.date),
          last.rd = as.integer(last.rd),
          best.rating = as.integer(best.rating),
          best.date = as.integer(best.date),
          record.win = as.integer(record.win),
          record.loss = as.integer(record.loss),
          record.draw = as.integer(record.draw)
        ) |>
    ...
    if (nrow(chessInfo) != 0) {
      chessInfo <- chessInfo |>
        mutate(type = time_class, end_time = as.Date(as.POSIXct(end_time, origin =
                                                                  "1970-01-01"))) |>
        mutate(
          white.user = white$username,
          black.user = black$username,
          white.rating = white$rating,
          black.rating = black$rating,
          white.result = white$result,
          black.result = black$result
        )
      if (chessType != "All") {
...
function(input, output, session) {
  output$table1 <- renderTable({
    queryChess(
      category = "player",
      chessType = input$gameType2,
      username = input$user2,
      subcategory = "stats"
    )
  })
  ...
  output$table2 <- renderDataTable({
    queryChess(
      category = "player",
      chessType = input$gameType3,
      username = input$user3,
      year = input$year3,
      month = input$month3,
      subcategory = "games"
    ) |>
  ...
  output$table3 <- renderDataTable({
    queryChess(
      ...
      chessType = input$gameType1,
      chessTitle = input$chessTitle1
    )
  })
  ...
  output$plot1 <- renderPlot(
    g() + geom_col(aes(fill = Record)) + labs(x = "Record", y = "Count", title =
                                                "Lifetime Statistics") + theme(legend.position = "none") + theme(plot.title = element_text(hjust =
                                                                                                                                             0.5))
  )
  ...
  h <- reactive({
    h <- ggplot(
      queryChess(
        ...
        mutate(userSide = ifelse(
          white.user == input$user3, "White", "Black"
        )) |>
        mutate(userELO = ifelse(
          userSide == "White", white.rating, black.rating
        )) |>
        select(userSide, userELO) |>
        group_by(userSide),
      aes(x = userSide, y = userELO)
    )
  })
  
  output$plot2 <- renderPlot(
    h() + geom_boxplot(aes(fill = userSide)) + labs(x = "Side Played", y = "ELO", title =
                                                      "ELO Ranking by Side Played") + theme(
  ...
  output$plot3 <- renderPlot(
    i() + geom_histogram(aes(fill = title), bins = input$bins) + labs(x = "ELO", y =
                                                                        "Count", title = "Leaderboard Ranking Distribution") + scale_fill_discrete(name = "Chess Title") +
  ...
  j <- reactive({
    i <- ggplot(
      queryChess(
        ...
        mutate(
          userELO = ifelse(userSide == "White", white.rating, black.rating),
          oppELO = ifelse(userSide != "White", white.rating, black.rating)
        ) |>
        select(userELO, oppELO, end_time) |>
        group_by(end_time) |>
        summarize(
          meanuserELO = mean(userELO),
          meanoppELO = mean(oppELO)
        ),
      aes(x = end_time)
    )
  })
  ...
  output$plot4 <- renderPlot(
    j() + geom_line(aes(y = meanuserELO, color = "User")) + geom_line(aes(y =
                                                                            meanoppELO, color = "Opponent")) + labs(
                                                                              x = "Date",
                                                                              y = "ELO",
                                                                              colour = "Player",
                                                                              title = "Comparison of ELO Fluctuation of Player and Opponents"
                                                                            ) + theme(plot.title = element_text(hjust = 0.5))
  )
  
  output$table5 <- renderTable(if (input$freqVar1 == "title") {
    queryChess(category = "leaderboards",
               chessType = "All",
               chessTitle = "All") |>
      select(type, title) |>
      filter(!is.na(title)) |>
      ...
      queryChess(
        ...
        mutate(userResult = ifelse(
          ...
          ifelse(
            ...
            "Draw",
            ifelse(
              userResult == "stalemate",
              "Draw",
              ifelse(
                userResult == "insufficient",
                "Draw",
                ifelse(userResult ==
                         "50move", "Draw", "Loss")
              )
  ...
  output$table8 <- renderTable({
    if (input$statVar1 == "User") {
      queryChess(
        ...
        group_by(Side) |>
        summarize(
          meanELO = mean(userELO),
          sdELO = sd(userELO),
          medianELO = median(userELO),
          iqrELO = IQR(userELO)
        ) |>
        arrange(desc(Side)) |>
  ...
  k <- reactive({
    k <- ggplot(
      ...
      aes(
        x = rank,
        y = score,
        color = get(input$facetVal1)
      )
    )
  })
  output$plot5 <- renderPlot({
    k() + geom_jitter() + facet_wrap( ~ get(input$facetVal1)) + labs(x = "Leaderboard Rankings",
                                                                     y = "ELO Ranking",
                                                                     color = input$facetVal1) + theme(plot.title = element_text(hjust = 0.5))
  })
}
